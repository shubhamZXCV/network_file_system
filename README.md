...
This repository contains the implementation of a Network File System (NFS) with a Naming Server, Storage Server, and Client. Below are the instructions to compile and run each component.
...
3. [Running the Servers and Client](#running-the-servers-and-client)
   - [Naming Server](#naming-server)
   - [Storage Server](#storage-server)
   - [Client](#client)
4. [Additional Notes](#additional-notes)
5. [Testing and Features](#testing-and-features)
...
- Networking tools like `ifconfig` to determine IP addresses.
- The `pthread` library for multithreading support.
- The `mpv` library for client-side functionality.
...
1. Navigate to the `naming_server` directory:
   ```bash
   cd naming_server
   ```
2. Compile the Naming Server with the following command:
   ```bash
   gcc namingServer/namingServer.c lru/lru.c log/log.c caching/caching.c path/path.c tries/tries.c -o nm -pthread -g
   ```

### **Storage Server**

1. Navigate to the `storage_server` directory:
   ```bash
   cd storage_server
   ```
2. Compile the Storage Server:
   ```bash
   gcc storage_server.c file.c network.c -o ss
   ```
...
1. Start the Naming Server:
   ...
   ./nm
   ```
2. Use `ifconfig` to find the IP address of the Naming Server, which will be needed by the Storage Server and Client:
   ```bash
   ifconfig
   ```
...
1. Run the Storage Server locally (127.0.0.1):
   ```bash
   ./ss <nm server port> <client port>
   ```
   Example:
   ```bash
   ./ss 5000 6000
   ```
2. Run the Storage Server with the Naming Server's IP address:
   ```bash
   ./ss <nm server port> <client port> <nm ip>
   ```
   Example:
   ```bash
   ./ss 5000 6000 192.168.1.2
   ```
...
1. Run the Client locally (127.0.0.1):
   ```bash
   ./client
   ```
2. Run the Client with the Naming Server's IP address:
   ```bash
   ./client <nm ip>
   ```
   Example:
...
- Ensure that all servers and the client are running on machines that can communicate with each other over the specified ports.
- Use unique port numbers for the Naming Server and Storage Server to avoid conflicts.
- Debugging flags (`-g`) are included in the compilation commands for troubleshooting if necessary.
- For detailed logs, enable any logging features built into the Naming Server or Storage Server.
...
> check out the ss-updated folder for the dir structure used below
